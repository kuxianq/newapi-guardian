...
async def handle_agent_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 检查 AI 是否启用
    if not get_enabled():
        await safe_reply(update.message, "🤖 AI 功能未启用。使用 `/ai_config enable` 打开。")
        return
    ...
    if has_prefix:
        ...
        if not user_prompt:
            await safe_reply(update.message, "⚡ 请在 @ai 后提供内容。例如 `@ai 帮我看下概览`。")
            return
    else:
        user_prompt = text
    ...
    if user_prompt.lower() in ["清空上下文", "clear context", "重新开始", "reset"]:
        ...
        agent.clear_context()
        await safe_reply(update.message, "✅ 已清空对话上下文，我们重新开始吧！")
        return
    ...
    try:
        ...
        if not result.get("success"):
            await safe_reply(update.message, f"❌ {result.get('message', '未知错误')}")
            return
        ...
        if not response:
            await safe_reply(update.message, "⚠️ AI 没有返回内容。")
            return
        ...
        await safe_reply(update.message, f"❌ 处理消息时出错：{str(e)}")
